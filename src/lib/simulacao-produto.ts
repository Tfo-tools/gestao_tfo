...
import { produtoEstaNoCenario } from "@/lib/fases-produto";
...
/**
 * Monta as premissas de um produto num cenário e roda a simulação — sem gravar nada. O recálculo
 * (simulacao-actions) grava o resultado; a prévia do plano pela receita usa isto com `override`
 * pra mostrar o antes × depois antes de salvar.
 */
...
/** Premissas do plano pela receita que ainda não foram salvas (prévia). */
export type OverridePlanoReceita = {
  fases: FasePlanoReceita[];
  sazonalidade: number[] | null;
  pctVendasCombo: number | null;
};

export type ResultadoSimulacaoProduto =
  | { error: string }
  | { foraDoCenario: true }
  | { resultado: MesResultado[] };

export type EntradaSimulacaoProduto =
  | { error: string }
  | { foraDoCenario: true }
  | { input: SimulacaoInput };

export async function simularProduto(
  supabase: Awaited<ReturnType<typeof createClient>>,
  produtoId: string,
  cenarioId: string,
  override?: OverridePlanoReceita | null,
): Promise<ResultadoSimulacaoProduto> {
  const e = await montarEntradaSimulacao(supabase, produtoId, cenarioId, override);
  if (!("input" in e)) return e;
  return { resultado: calcularSimulacao(e.input) };
}

/** As premissas do produto no cenário, prontas pro motor. `modeloTrimestral` força o modelo
 *  anterior (usado pra calibrar a conversão), mesmo que o cenário já planeje pela receita. */
export async function montarEntradaSimulacao(
  supabase: Awaited<ReturnType<typeof createClient>>,
  produtoId: string,
  cenarioId: string,
  override?: OverridePlanoReceita | null,
  modeloTrimestral = false,
): Promise<EntradaSimulacaoProduto> {
  ...
  const [
    ...
    supabase
      .from("produtos")
      .select(
        "data_inicio_desenvolvimento, data_lancamento_estimada, tipo_precificacao, tem_implementacao, preco_implementacao, implementacao_parcelas, implementacao_formas_pagamento, sazonalidade_vendas",
      )
      .eq("id", produtoId)
      .single(),
    supabase
      .from("fases_produto")
      .select(
        "id, fase, data_inicio, data_fim, taxa_crescimento_mensal, taxa_churn_mensal, plano_cresc_inicio, plano_cresc_alvo, plano_churn_inicio, plano_churn_alvo, fases_trimestres(indice, taxa_crescimento_mensal, taxa_churn_mensal)",
      )
  ...
  if (!noCenario) {
    // Não é erro: este cenário não simula este produto.
    return { foraDoCenario: true };
  }
  ...
  const { data: cenario } = await supabase
    .from("cenarios")
    .select("data_fim, ponto_partida, modelo_plano, pct_vendas_combo")
    .eq("id", cenarioId)
    .single();
  // Plano pela receita: o cenário já está nele, ou é a prévia de uma edição (override).
  const pelaReceita = !modeloTrimestral && (!!override || cenario?.modelo_plano === "receita");
  const pctVendasCombo: number | null = override
    ? override.pctVendasCombo
    : cenario?.pct_vendas_combo != null
      ? Number(cenario.pct_vendas_combo)
      : null;
  const numOuNull = (v: unknown) => (v != null ? Number(v) : null);
  ...
  const input: SimulacaoInput = {
    ...
    pontoPartida,
    vendasAcoes,
    planoReceita: pelaReceita
      ? {
          fases:
            override?.fases ??
            fases.map((f) => ({
              fase: f.fase as FaseValue,
              cresc_inicio: numOuNull(f.plano_cresc_inicio),
              cresc_alvo: numOuNull(f.plano_cresc_alvo),
              churn_inicio: numOuNull(f.plano_churn_inicio),
              churn_alvo: numOuNull(f.plano_churn_alvo),
            })),
          sazonalidade: override
            ? override.sazonalidade
            : Array.isArray(produto.sazonalidade_vendas)
              ? (produto.sazonalidade_vendas as unknown[]).map(Number)
              : null,
        }
      : null,
    fases: fases.map((f) => ({
    ...
    // Modelo trimestral: só os combos com % preenchido no cadastro, como sempre foi. Plano pela
    // receita: o % de vendas em combo é do cenário (o desconto continua vindo do cadastro).
    combos: (combosRaw ?? [])
      .filter(
        (combo) => pelaReceita || combo.percentual_clientes_combo != null,
      )
      .map((combo) => {
      ...
      return {
        percentual_clientes_combo: pelaReceita
          ? (pctVendasCombo ?? Number(combo.percentual_clientes_combo ?? 0))
          : Number(combo.percentual_clientes_combo),
