...
export function infraBaseNoMes(
  infra: CogsPremissas["infra"],
  clientes: number,
): number {
  const degraus = [...(infra?.degraus ?? [])].sort(
    (a, b) => a.a_partir_de_clientes - b.a_partir_de_clientes,
  );
  let base = infra?.base_mensal ?? 0;
  for (const d of degraus)
    if (clientes >= d.a_partir_de_clientes) base = d.base_mensal;
  return base;
}
...
export function horasCsPorCliente(c: CogsPremissas["cs_proativo"]): number {
  if (!c?.ativo) return 0;
  return (
    ((c.monitoramento_h ?? 0) + (c.cadencia_h ?? 0) + (c.qbr_h_mes ?? 0)) *
    (c.overhead ?? 1)
  );
}
...
export function custoLlmPorCliente(l: CogsPremissas["llm"]): number {
  if (!l?.ativo) return 0;
  const entrada =
    ((l.tokens_entrada_mes ?? 0) / 1_000_000) *
    (l.preco_milhao_entrada_usd ?? 0);
  const saida =
    ((l.tokens_saida_mes ?? 0) / 1_000_000) * (l.preco_milhao_saida_usd ?? 0);
...
export function calcularCogsMes(
  p: CogsPremissas | null | undefined,
  e: CogsEstadoMes,
): CogsResultado {
  const vazio: CogsResultado = {
    infraestrutura: 0,
    llm: 0,
    suporteReativo: 0,
    csProativo: 0,
  ...
  const infraestrutura =
    regraAtiva(p.infra?.inicio, e.mes) &&
    (n > 0 || (p.infra?.base_mensal ?? 0) > 0)
      ? infraBaseNoMes(p.infra, n) + n * (p.infra?.por_cliente_mes ?? 0)
      : 0;

  // 1.1.2 — só a fatia da base que está no nível com LLM.
  const llm =
    n * Math.max(0, Math.min(1, e.fracaoNivelLlm)) * custoLlmPorCliente(p.llm);
  ...
  const suporteReativo = regraAtiva(p.suporte?.inicio, e.mes)
    ? horasSuporte * e.custoHora(p.suporte ?? {})
    : 0;
  const horasCs = n * horasCsPorCliente(p.cs_proativo);
  const csProativo = regraAtiva(p.cs_proativo?.inicio, e.mes)
    ? horasCs * e.custoHora(p.cs_proativo ?? {})
    : 0;
  ...
  const g = p.gateway;
  const somaMix =
    (g?.mix_cartao ?? 0) + (g?.mix_boleto ?? 0) + (g?.mix_pix ?? 0);
  let gateway = 0;
  if (g && somaMix > 0 && e.transacoes > 0) {
    ...
    gateway =
      e.transacoes * fc * (g.cartao_fixo ?? 0) +
      e.receitaCobrada * fc * (g.cartao_pct ?? 0) +
...
export function horasAtendimentoPorProduto(
  ...
  const out: Record<string, number> = {};
  for (const l of linhas) {
    out[l.produto_id] = horasSuportePorCliente(l.parametros?.suporte);
  }
  return out;
}
