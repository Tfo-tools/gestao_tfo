/* eslint-disable @typescript-eslint/no-explicit-any -- cliente Supabase sem tipos gerados */
/**
 * Simulação a partir do FUNSES 1: recalibra um cenário como fração do FUNSES 1 (CLIENTES ativos, mês a mês),
...
import {
  montarEntradaSimulacao,
  simularProduto,
} from "@/lib/simulacao-produto";
import {
  calibrarCrescimento,
...
import {
  ...
  horizonte,
  type FaseLinha,
} from "@/lib/plano-receita-cenario";
...
import {
  horasAtendimentoPorProduto,
  horasCsProativoPorProduto,
} from "@/lib/cogs";
...
const MODELO_PJ_VARIAVEL_NOME =
  "Vendedor PJ — variável (proporcional à demanda)";
  ...
  return forma.map((f) => {
    if (f.fase === "maturidade") {
      return {
        ...
        churn_alvo:
          f.churn_alvo != null
            ? churnMensalParaAnual(churnAnualParaMensal(f.churn_alvo) * k)
            : null,
      };
    }
    return {
...
export async function executarSimulacaoCenarios(
  admin: any,
  CENARIOS: ConfigSimulacao[],
): Promise<{ log: string[]; backup: unknown }> {
  const log: string[] = [];
  const escrever = (s: string) => log.push(s);
  if (CENARIOS.some((c) => c.nome === "FUNSES 1"))
    throw new Error("O FUNSES 1 não pode ser alterado.");
  ...
  // A premissa é sobre CLIENTES: "50% do FUNSES 1" = metade dos clientes ativos, mês a mês, por
  // produto. Preço é premissa à parte — cobrar mais aparece como receita maior, não como menos
  // clientes.
  const { data: f1Sim } = await admin
  ...
  const { data: mindRow } = await admin
    .from("produtos")
    .select("id")
    .eq("nome", PRODUTO_VENDEDOR)
    .single();
  ...
  const backup = {
    cenarios: cenIds,
    fases_produto: bkFases,
    alocacoes: bkAloc,
  };
  const { error: erroBackup } = await admin
    .from("simulacoes_backup")
    .insert({
      descricao: CENARIOS.map(
        (c) => `${c.nome} ${c.indice}× churn ${c.churnMensal ?? "FUNSES 1"}`,
      ).join(" | "),
      conteudo: backup,
    });
  if (erroBackup)
    throw new Error(
      `Não consegui guardar a cópia de segurança: ${erroBackup.message}`,
    );
  ...
  for (const cfg of CENARIOS) {
    ...
    for (const p of produtos) {
      ...
      const motorRef: FaseInput[] = temRef
        ? fasesDoMotor(linhasF1 as FaseLinha[], pF1!.fases)
        : motor;
      const mesesRef = temRef
        ? mesesComFase(motorRef, horizonte(motorRef, fim))
        : meses;
      let forma = converterDoModeloTrimestral({
        ...
        taxaLegada: (fase, mes) => {
          ...
          return f
            ? taxasDoMes(f, new Date(mes + "T00:00:00"))
            : { crescimento: 0, churn: 0 };
        },
        mrrPorMes: new Map(),
      });
      ...
      for (const fase of FASES_CHURN) {
        if (
          !forma.some((f) => f.fase === fase) &&
          meses.some((m) => m.fase === fase)
        ) {
          ...
          forma = [
            ...forma,
            {
              ...
              churn_alvo:
                fase === "maturidade"
                  ? churnMensalParaAnual(
                      ult?.churn_alvo ?? cfg.churnMensal ?? 0.02,
                    )
                  : (ult?.churn_alvo ?? cfg.churnMensal ?? 0.02),
            },
          ];
      ...
      const { data: prod } = await admin
        .from("produtos")
        .select("sazonalidade_vendas")
        .eq("id", p.id)
        .single();
      const saz = Array.isArray(prod?.sazonalidade_vendas)
        ? (prod.sazonalidade_vendas as unknown[]).map(Number)
        : null;
      const pct =
        cen.pct_vendas_combo != null ? Number(cen.pct_vendas_combo) : null;
      const entrada = await montarEntradaSimulacao(admin, p.id, cen.id, {
        fases: forma,
        sazonalidade: saz,
        pctVendasCombo: pct,
      });
    ...
    // médio ponderado do período; como os clientes ficam presos ao alvo, a média é quase
    ...
    for (let passo = 0; passo < 4; passo++) {
      ...
      for (const pr of preps) {
        ...
        // O calibrador compara o campo `mrr` com o alvo; aqui o alvo é clientes, então entrega
        // clientes_ativos nesse campo.
        const fases = calibrarCrescimento({
          simular: (f) =>
            calcularSimulacao({
              ...pr.input,
              planoReceita: { fases: f, sazonalidade: pr.saz },
            }).map((r) => ({ mes_referencia: r.mes_referencia, mrr: r.clientes_ativos })),
        ...
        for (const r of calcularSimulacao({
          ...pr.input,
          planoReceita: { fases, sazonalidade: pr.saz },
        }))
          linhasSim.push({
            mes_referencia: r.mes_referencia,
            clientes_ativos: r.clientes_ativos,
            churn_pct: r.churn_pct,
          });
      }
      medido = churnMedioPonderado(linhasSim, inicioCen, fim);
      escrever(
        `  passo ${passo + 1}: k=${k.toFixed(3)} → churn médio ${(medido * 100).toFixed(2)}%`,
      );
      if (
        cfg.churnMensal == null ||
        Math.abs(medido - cfg.churnMensal) < 0.0002 ||
        medido <= 0
      )
        break;
    ...
    if (cen.modelo_plano !== "receita")
      await admin
        .from("cenarios")
        .update({ modelo_plano: "receita" })
        .eq("id", cen.id);
    ...
    if (cfg.regraVendedor) {
      const [
        ...
        admin
          .from("simulacao_mensal")
          .select(
            "produto_id, mes_referencia, novos_clientes, clientes_ativos, receita_bruta, novos_direto, novos_representante, novos_associacao, novos_acoes",
          )
          .eq("cenario_id", cen.id),
        admin
          .from("canais_aquisicao")
          .select(
            "id, tipo_canal, modelo_contratacao_id, canal_produto(produto_id, percentual_mix, taxa_fechamento)",
          )
          .eq("cenario_id", cen.id),
        admin
          .from("cogs_premissas")
      ...
      const faseById = new Map(
        (fasesRaw ?? []).map((f: { id: string }) => [f.id, f]),
      );
        ...
        .map((f: any) => {
          ...
          return {
            ...
            capacidade_vendedor_mes:
              f.capacidade_vendedor_mes != null
                ? Number(f.capacidade_vendedor_mes)
                : null,
            span_of_control:
              f.span_of_control != null ? Number(f.span_of_control) : null,
            horas_suporte_por_cliente_mes:
              f.horas_suporte_por_cliente_mes != null
                ? Number(f.horas_suporte_por_cliente_mes)
                : null,
            reunioes_por_oportunidade:
              f.reunioes_por_oportunidade != null
                ? Number(f.reunioes_por_oportunidade)
                : null,
          } as FunilPremissaInput;
        })
        .filter(
          (f: FunilPremissaInput | null): f is FunilPremissaInput => f !== null,
        );
      const simulacao: SimulacaoMesInput[] = (simRaw ?? []).map((s: any) => ({
        ...
        novos_direto:
          s.novos_direto != null ? Number(s.novos_direto) : undefined,
        novos_representante:
          s.novos_representante != null
            ? Number(s.novos_representante)
            : undefined,
        novos_associacao:
          s.novos_associacao != null ? Number(s.novos_associacao) : undefined,
        novos_acoes: s.novos_acoes != null ? Number(s.novos_acoes) : undefined,
      }));
      const qualif = new Map(
        (modelos ?? []).map((m: any) => [
          m.id,
      ...
      const canais: CanalFunilInput[] = (canaisRaw ?? []).flatMap((c: any) =>
        (c.canal_produto ?? []).map((cp: any) => ({
          ...
          taxa_qualificacao: c.modelo_contratacao_id
            ? (qualif.get(c.modelo_contratacao_id) ?? null)
            : null,
        })),
      );
      const demanda = calcularDemandaPorCargo({
        ...
        canais,
        simulacao,
        horasSuportePorProduto: horasAtendimentoPorProduto(
          (cogsRaw ?? []) as any,
        ),
      ...
      for (const [mes, d] of Object.entries(demanda.porProduto[mindId] ?? {}))
        necessidade.set(mes, d.vendedores);

      // 0 = PJ proporcional (até 1,4); n>=1 = n CLT (1,5→1, 2,5→2, 3,5→3…)
      const cltDe = (n: number) => (n < 1.5 ? 0 : Math.floor(n - 0.5));
      let { data: pjVar } = await admin
        .from("modelos_contratacao")
        .select("id")
        .eq("nome", MODELO_PJ_VARIAVEL_NOME)
        .maybeSingle();
      if (!pjVar) {
      ...
      escrever(
        "  vendedor (Mind): " +
          faixas
            .map(
              (f) =>
                `${f.inicio.slice(0, 7)}→${f.fim.slice(0, 7)} ${f.qtd === 0 ? "PJ proporcional" : `${f.qtd} CLT`}`,
            )
            .join(" | "),
      );
    } else {
      escrever(
        "  vendedor: alocação mantida como está (regras atuais do cenário)",
      );
    }
    ...
    const { data: novo } = await admin
      .from("simulacao_mensal")
      .select("mes_referencia, clientes_ativos, mrr, churn_pct")
      .eq("cenario_id", cen.id);
    ...
    for (const r of novo ?? []) {
      ...
      porAno.set(a, {
        cli: x.cli + Number(r.clientes_ativos ?? 0),
        mrr: x.mrr + Number(r.mrr ?? 0),
      });
    }
    ...
    escrever(
      `  churn médio do período (como o card): ${(churnFinal * 100).toFixed(2)}% a.m.`,
    );
    for (const [a, x] of [...porAno.entries()].sort()) {
      const f1Cli = [...f1Por.values()].reduce(
        (s, m) => s + (m.get(`${a}-12-01`) ?? 0),
        0,
      );
      escrever(
