...
export async function executarSimulacaoCenarios(
  ...
  const { data: f1Sim } = await admin
    ...
    .select("produto_id, mes_referencia, mrr")
    .eq("cenario_id", f1.id);
  ...
  for (const cfg of CENARIOS) {
    ...
    for (const [a, x] of [...porAno.entries()].sort()) {
      const f1Mrr = [...f1Por.values()].reduce(
        (s, m) => s + (m.get(`${a}-12-01`) ?? 0),
        0,
      );
      escrever(
        `  dez/${a}: ${Math.round(x.cli)} clientes · MRR ${Math.round(x.mrr)}${f1Mrr > 0 ? ` (${((x.mrr / f1Mrr) * 100).toFixed(1)}% do FUNSES 1)` : ""}${cfg.regraVendedor ? ` · necessidade vendedor Mind ${(necessidade.get(`${a}-12-01`) ?? 0).toFixed(2)}` : ""}`,
      );
    }
  }
  return { log, backup };
