 ...
 *   (clientes por produto e plano que se espera fechar na feira). O custo é pago em 3 parcelas iguais
 *   até o mês da feira (M−2, M−1 e M); as vendas são do mês da feira, no canal direto.
...
export const LABEL_TIPO_ACAO: Record<TipoAcao, string> = { feira: "Feira", evento: "Eventos", campanha: "Campanha" };
...
export function mesesDaCampanha(a: AcaoMarketing, fimCenario?: string | null): string[] {
  if (a.tipo !== "campanha" || !a.mes) return [];
  const fim = `${(a.mes_fim ?? fimCenario ?? FIM_PADRAO_CAMPANHA).slice(0, 7)}-01`;
  const meses: string[] = [];
  for (let m = `${a.mes.slice(0, 7)}-01`; m <= fim && meses.length < 600; m = somarMeses(m, 1)) meses.push(m);
  return meses;
}
...
export function custoTotalAcao(a: AcaoMarketing, fimCenario?: string | null): number {
  if (a.tipo === "feira") return Number(a.custo);
  if (a.tipo === "campanha") return Number(a.custo) * mesesDaCampanha(a, fimCenario).length;
  return Number(a.custo) * Number(a.quantidade ?? 0);
}

/** Custo da ação por mês (AAAA-MM-01). */
export function custoAcaoPorMes(a: AcaoMarketing, fimCenario?: string | null): Map<string, number> {
  ...
  if (a.tipo === "feira") {
    ...
    for (const m of mesesDaCampanha(a, fimCenario)) porMes.set(m, Number(a.custo));
  } else if (a.ano) {
...
export function vendasAcaoPorMes(
  ...
  if (a.tipo === "campanha") {
    ...
    const pesoTotal = linhas.reduce((s, r) => s + (Number(r.clientes) > 0 ? Number(r.clientes) : 1), 0);
    const porMes = clientesMensaisCampanha(a);
    if (pesoTotal <= 0 || porMes <= 0) return [];
    return linhas
      .filter((r) => r.produto_id === produtoId)
      .flatMap((r) => {
        const parte = (Number(r.clientes) > 0 ? Number(r.clientes) : 1) / pesoTotal;
        return mesesDaCampanha(a, fimCenario).map((mes) => ({ mes, clientes: porMes * parte, retorno: r }));
      });
  }
...
export function clientesTotaisAcao(a: AcaoMarketing, fimCenario?: string | null): number {
  if (a.tipo === "campanha") return clientesMensaisCampanha(a) * mesesDaCampanha(a, fimCenario).length;
...
export function clientesAcoesPorProdutoMes(acoes: AcaoMarketing[], fimCenario?: string | null): Map<string, Map<string, number>> {
  const out = new Map<string, Map<string, number>>();
  for (const a of acoes) {
    const produtos = new Set((a.retorno ?? []).map((r) => r.produto_id).filter(Boolean));
    for (const pid of produtos) {
      const doProduto = out.get(pid) ?? new Map<string, number>();
      for (const v of vendasAcaoPorMes(a, pid, fimCenario)) doProduto.set(v.mes, (doProduto.get(v.mes) ?? 0) + v.clientes);
