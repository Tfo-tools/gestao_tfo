...
const LABEL_COBRANCA: Record<string, string> = { anual: "compromisso de 12 meses", mensal: "sem fidelidade" };

/** Total pago no ciclo de compromisso — `preco` é sempre o valor MENSAL. */
function totalDoCompromisso(preco: number, tipoCobranca: string | null): string | null {
  if (tipoCobranca !== "anual") return null;
  return `= ${formatBRL(preco * 12)} em 12 parcelas`;
}

export function NiveisModulo({ produtoId, cenarioId, niveis }: { produtoId: string; cenarioId: string; niveis: Nivel[] }) {
  const [drawerAberto, setDrawerAberto] = useState(false);
  const [isPending, startTransition] = useTransition();

  const niveisOrdenados = [...niveis].sort((a, b) => (a.ordem ?? 0) - (b.ordem ?? 0));
  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-5">
      ...
      <p className="mb-4 text-[11px] text-text-muted">
        Nível 1 é o plano base do produto — os próximos níveis são os módulos/combos que vão sendo lançados ao longo do tempo.
      </p>

      <div className="flex flex-col gap-3">
        {niveisOrdenados.length === 0 && (
          <div className="flex flex-col items-start gap-2 rounded-lg border border-dashed border-border-soft px-4 py-5">
            <p className="text-[12px] text-text-faint">Nenhum nível cadastrado ainda — comece pelo plano base (nível 1).</p>
            <button
              type="button"
              onClick={() => setDrawerAberto(true)}
        ...
        {niveisOrdenados.map((n) => (
          <div key={n.id} className="rounded-lg border border-border-soft px-4 py-3">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="mt-1 text-[10.5px] text-text-faint">
                  ...
                  {n.tipo_cobranca ? ` · ${LABEL_COBRANCA[n.tipo_cobranca] ?? n.tipo_cobranca}` : ""}
                  {n.quantidade_usuarios_inclusos != null ? ` · ${n.quantidade_usuarios_inclusos} usuário(s) inclusos` : ""}
                  {n.preco_usuario_adicional != null ? ` · +${formatBRL(Number(n.preco_usuario_adicional))}/usuário adicional` : ""}
                </div>
                <div className="mt-0.5 text-[10.5px] text-text-faint">
                  adesão inicial {(n.adesao_inicial_pct * 100).toFixed(1)}% · +{(n.crescimento_adesao_mensal_pct * 100).toFixed(1)}%/mês
                  {n.percentual_permanencia_estimado != null
                    ? ` · estabiliza em ${(n.percentual_permanencia_estimado * 100).toFixed(1)}% dos clientes`
                    : ""}
                  {n.media_usuarios_por_cliente != null ? ` · média de ${n.media_usuarios_por_cliente} usuários/cliente` : ""}
                  {n.desconto_cliente_existente_pct != null &&
                    ` · cliente já existente: ${(n.desconto_cliente_existente_pct * 100).toFixed(1)}% off${
                      n.desconto_cliente_existente_meses != null ? ` por ${n.desconto_cliente_existente_meses}m` : " (permanente)"
                    }`}
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-right">
                  <span className="block font-mono text-[13px] font-semibold">{formatBRL(Number(n.preco))}/mês</span>
                  {totalDoCompromisso(Number(n.preco), n.tipo_cobranca) && (
                    <span className="block text-[9.5px] text-text-faint">{totalDoCompromisso(Number(n.preco), n.tipo_cobranca)}</span>
                  )}
                </span>
                <button
                  type="button"
                  disabled={isPending}
                  onClick={() => startTransition(() => excluirModulo(n.id, produtoId))}
            ...
            <BetaModuloSection produtoId={produtoId} moduloId={n.id} itens={n.betaTesters} />
          </div>
        ))}
      </div>
...
function DrawerNivel({
  ...
  const [state, formAction, pending] = useActionState(criarNivelModulo, initialState);
      ...
      <div className="fixed inset-y-0 left-0 z-50 flex h-full w-full max-w-[420px] flex-col overflow-y-auto bg-surface shadow-xl md:w-1/3">
        <div className="flex items-center justify-between border-b border-border-soft px-5 py-4">
          <h3 className="font-heading text-[14px] font-semibold">
            {primeiroNivel ? "Novo plano base (Nível 1)" : `Novo módulo (Nível ${ordem})`}
          </h3>
          <button type="button" onClick={onClose} className="text-[13px] text-text-muted">
            ✕
          </button>
        </div>

        <form ref={formRef} action={formAction} className="flex flex-1 flex-col gap-3 overflow-y-auto px-5 py-4">
          ...
          <div>
            ...
            <input name="nome" placeholder="Ex: Basic, Starter, Premium" className="input" required />
          </div>

          <div className="grid grid-cols-2 gap-2.5">
            <div>
              <label className="mb-1 block text-[10.5px] font-medium text-text-muted">Data de lançamento</label>
              <input name="data_disponibilidade" type="date" className="input" required />
            </div>
            {primeiroNivel ? (
              <div>
                ...
                <select name="tipo_cobranca" className="input" defaultValue="" required>
              ...
              <div>
                <label className="mb-1 block text-[10.5px] font-medium text-text-muted">Usuários inclusos</label>
                <input name="quantidade_usuarios_inclusos" type="number" step="1" min="0" className="input" />
              </div>
            )}
          </div>
          ...
          {primeiroNivel && (
            <div>
              <label className="mb-1 block text-[10.5px] font-medium text-text-muted">Quantidade de usuários no plano</label>
              <input name="quantidade_usuarios_inclusos" type="number" step="1" min="0" className="input" />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              ...
              <input name="preco" type="number" step="0.01" placeholder="Ex: 489" className="input" required />
            </div>
            <div>
              <label className="mb-1 block text-[10.5px] font-medium text-text-muted">Preço usuário adicional (R$)</label>
              <input name="preco_usuario_adicional" type="number" step="0.01" className="input" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              ...
              <input name="media_usuarios_por_cliente" type="number" step="0.01" className="input" />
            </div>
            <div>
              <label className="mb-1 flex items-center text-[10.5px] font-medium text-text-muted">
                % que permanece
              ...
              <input name="percentual_permanencia_estimado" type="number" step="0.01" placeholder="Ex: 100" className="input" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              ...
              <input name="adesao_inicial_pct" type="number" step="0.01" defaultValue={100} className="input" />
            </div>
            <div>
              <label className="mb-1 flex items-center text-[10.5px] font-medium text-text-muted">
                Crescimento mensal (%)
              ...
              <input name="crescimento_adesao_mensal_pct" type="number" step="0.01" defaultValue={0} className="input" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2.5">
            <div>
              ...
              <input name="desconto_cliente_existente_pct" type="number" step="0.01" className="input" />
            </div>
            <div>
              <label className="mb-1 block text-[10.5px] font-medium text-text-muted">Duração do desconto (meses)</label>
          ...
          <p className="mt-1 flex items-center text-[10.5px] text-text-faint">
            Beta testers deste nível são sempre empresas, nunca pessoas — cadastre depois de salvar, no card do nível.
          </p>
          ...
          <div className="mt-auto flex gap-2 border-t border-border-soft pt-4">
            ...
            <button type="button" onClick={onClose} className="rounded-lg border border-border px-4 py-2 text-[12.5px] text-text-muted">
              Cancelar
            </button>
          </div>
        </form>
