...
export default async function CenariosPage() {
  ...
  const [{ data: cenarios }, { data: programas }, { data: vinculos }] = await Promise.all([
    supabase.from("cenarios").select("id, nome, descricao, is_base, status, data_inicio, data_fim, created_at").order("created_at"),
    supabase.from("programas_investimento").select("id, nome, tipo").order("created_at"),
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <NovoCenarioForm
          cenarios={(cenarios ?? []).map((c) => ({ id: c.id, nome: c.nome, is_base: c.is_base, data_inicio: c.data_inicio, data_fim: c.data_fim }))}
          programas={programas ?? []}
