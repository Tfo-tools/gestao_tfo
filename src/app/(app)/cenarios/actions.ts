...
export async function renomearCenario(
  ...
  const { data: antes } = await supabase.from("cenarios").select("data_inicio, data_fim, cenario_origem_id").eq("id", id).single();

  const { error } = await supabase
    .from("cenarios")
    .update({ nome, descricao, data_inicio: `${mesInicio}-01`, data_fim: `${mesFim}-01` })
    .eq("id", id);
  ...
  if (mudouInicio && antes?.cenario_origem_id) {
    const ponto = await capturarPontoPartida(supabase, antes.cenario_origem_id, `${mesInicio}-01`);
    await supabase.from("cenarios").update({ ponto_partida: ponto }).eq("id", id);
  }
...
export async function criarCenario(
  ...
  const programaIds = [...new Set(formData.getAll("programa_id").map(String).filter(Boolean))];
  const mesInicio = String(formData.get("data_inicio") || "");
  const mesFim = String(formData.get("data_fim") || "");
  ...
  if (programaIds.length > 0) {
    await supabase.from("cenario_programas").insert(programaIds.map((programa_id) => ({ cenario_id: novoCenario.id, programa_id })));
  }

  const falhas: string[] = [];
  if (duplicarDe) {
    const mapProdutoId = await clonarCenario(supabase, duplicarDe, novoCenario.id, falhas);
    await copiarConfiguracoesDeCenario(supabase, duplicarDe, novoCenario.id, mapProdutoId);

    // Abertura do cenário = base de clientes que a origem tinha no mês anterior ao início escolhido.
    const ponto = await capturarPontoPartida(supabase, duplicarDe, `${mesInicio}-01`, (id) => mapProdutoId(id) ?? id);
    if (ponto) await supabase.from("cenarios").update({ ponto_partida: ponto }).eq("id", novoCenario.id);

    // Sem recalcular, o cenário novo abria sem projeção nenhuma (ou com a de outro momento).
    const { falhas: falhasCalculo } = await recalcularTodosProdutos(novoCenario.id);
...
async function clonarCenario(
  ...
  const inserirVarios = async (tabela: string, linhas: Row[], rotulo: string) => {
  ...
  const { data: produtosCenarioOrigem } = await supabase.from("produtos").select("*").eq("cenario_id", origemId);
  ...
  const mapProdutoId = (id: string | null) => (id ? (produtoIdMap.get(id) ?? id) : id);

  // Fases + tudo que pendura nelas (funil, custos da fase, equipe, trimestres de crescimento/churn).
  const { data: fasesOrigem } = await supabase.from("fases_produto").select("*").eq("cenario_id", origemId);
  for (const fase of (fasesOrigem ?? []) as Row[]) {
    const { data: novaFase } = await supabase
      .from("fases_produto")
      .insert({ ...semColunasDeSistema(fase), produto_id: mapProdutoId(fase.produto_id), cenario_id: destinoId })
      .select("id")
      .single();
    if (!novaFase) {
    ...
    const filhas = ["premissas_funil", "plano_custos_fixos", "plano_custos_variaveis", "equipe_alocada", "fases_trimestres"];
    const resultados = await Promise.all(filhas.map((t) => supabase.from(t).select("*").eq("fase_produto_id", fase.id)));
    for (const [i, tabela] of filhas.entries()) {
      const linhas = ((resultados[i].data ?? []) as Row[]).map((r) => ({ ...semColunasDeSistema(r), fase_produto_id: novaFase.id }));
      await inserirVarios(tabela, linhas, `${tabela.replace(/_/g, " ")} (${fase.fase})`);
    }
  }

  // Tabelas presas direto ao cenário, sem filhas.
  for (const tabela of ["contratacoes", "beta_testers_config", "alocacao_modelo_contratacao", "alocacao_investimento"]) {
    const { data } = await supabase.from(tabela).select("*").eq("cenario_id", origemId);
  ...
  const { data: custosEmpresaOrigem } = await supabase.from("custos_empresa").select("*").eq("cenario_id", origemId);
  await inserirVarios(
    "custos_empresa",
    ((custosEmpresaOrigem ?? []) as Row[]).map((c) => {
      const parametros = c.parametros ? { ...c.parametros } : c.parametros;
      if (parametros?.produto_referencia_id) parametros.produto_referencia_id = mapProdutoId(parametros.produto_referencia_id);
  ...
  const { data: planosOrigem } = await supabase.from("planos_precificacao").select("*").eq("cenario_id", origemId);
  for (const plano of (planosOrigem ?? []) as Row[]) {
    const { data: novoPlano } = await supabase
      .from("planos_precificacao")
      .insert({ ...semColunasDeSistema(plano), produto_id: mapProdutoId(plano.produto_id), cenario_id: destinoId })
      .select("id")
      .single();
    if (!novoPlano) {
    ...
    const { data: precosFase } = await supabase.from("planos_precificacao_fases").select("*").eq("plano_id", plano.id);
    await inserirVarios(
      "planos_precificacao_fases",
      ((precosFase ?? []) as Row[]).map((pf) => ({ ...semColunasDeSistema(pf), plano_id: novoPlano.id })),
      "preço por fase",
    );
  }
...
async function clonarModulos(
  ...
  const { data: modulosOrigem } = await supabase.from("modulos_produto").select("*").eq("cenario_id", origemId);
  for (const modulo of (modulosOrigem ?? []) as Row[]) {
    ...
    const { data: novoModulo } = await supabase
      ...
      .insert({ ...semColunasDeSistema(modulo), produto_id: mapProdutoId(modulo.produto_id), cenario_id: destinoId })
      .select("id")
      .single();
    if (!novoModulo) {
    ...
    const { data: betas } = await supabase.from("beta_testers_modulo").select("*").eq("modulo_id", modulo.id);
    const linhas = ((betas ?? []) as Row[]).map((b) => ({ ...semColunasDeSistema(b), modulo_id: novoModulo.id }));
    if (linhas.length > 0) {
      const { error } = await supabase.from("beta_testers_modulo").insert(linhas);
...
export async function copiarConfiguracoesDeCenario(
  ...
  const { data: canaisDestino } = await supabase.from("canais_aquisicao").select("id").eq("cenario_id", destinoId);
  if (!canaisDestino || canaisDestino.length === 0) {
    const { data: canaisOrigem } = await supabase.from("canais_aquisicao").select("*").eq("cenario_id", origemId);
    for (const c of (canaisOrigem ?? []) as Row[]) {
      ...
      if (matriz && matriz.length > 0) {
        await supabase
          .from("canal_produto")
          .insert((matriz as Row[]).map((m) => ({ ...semColunasDeSistema(m), canal_id: novo.id, produto_id: mapProdutoId(m.produto_id) })));
      }
      if (parceiros && parceiros.length > 0) {
        await supabase.from("canal_parceiros_fase").insert((parceiros as Row[]).map((p) => ({ ...semColunasDeSistema(p), canal_id: novo.id })));
      }
    }
  }

  // Regras de COGS por produto
  const { data: cogsDestino } = await supabase.from("cogs_premissas").select("id").eq("cenario_id", destinoId);
  if (!cogsDestino || cogsDestino.length === 0) {
    const { data: cogsOrigem } = await supabase.from("cogs_premissas").select("*").eq("cenario_id", origemId);
  ...
  const { data: etapasDestino } = await supabase.from("implementacao_etapas").select("id").eq("cenario_id", destinoId);
  if (!etapasDestino || etapasDestino.length === 0) {
    const { data: etapasOrigem } = await supabase.from("implementacao_etapas").select("*").eq("cenario_id", origemId);
    if (etapasOrigem && etapasOrigem.length > 0) {
      await supabase
        .from("implementacao_etapas")
        .insert((etapasOrigem as Row[]).map((e) => ({ ...semColunasDeSistema(e), produto_id: mapProdutoId(e.produto_id), cenario_id: destinoId })));
    }
  }
  ...
  const [{ data: fasesOrigem }, { data: fasesDestino }] = await Promise.all([
    supabase.from("fases_produto").select("id, produto_id, fase").eq("cenario_id", origemId),
    supabase.from("fases_produto").select("id, produto_id, fase").eq("cenario_id", destinoId),
  ]);
  const destinoPorChave = new Map((fasesDestino ?? []).map((f) => [`${f.produto_id}__${f.fase}`, f.id]));
  for (const fo of fasesOrigem ?? []) {
    const destinoId2 = destinoPorChave.get(`${mapProdutoId(fo.produto_id)}__${fo.fase}`);
    if (!destinoId2) continue;
    const { data: jaTem } = await supabase.from("fases_trimestres").select("id").eq("fase_produto_id", destinoId2).limit(1);
    if (jaTem && jaTem.length > 0) continue;
    const { data: tri } = await supabase.from("fases_trimestres").select("indice, taxa_crescimento_mensal, taxa_churn_mensal").eq("fase_produto_id", fo.id);
    if (tri && tri.length > 0) await supabase.from("fases_trimestres").insert(tri.map((t) => ({ ...t, fase_produto_id: destinoId2 })));
  }
}
 ...
 */
export async function completarCenarioClonado(cenarioId: string): Promise<{ error: string | null; aviso?: string }> {
  const supabase = await createClient();
  const { data: c } = await supabase.from("cenarios").select("id, cenario_origem_id").eq("id", cenarioId).single();
  if (!c?.cenario_origem_id) return { error: "Este cenário não foi criado a partir de outro." };
  ...
  const [{ data: exclusivosOrigem }, { data: exclusivosDestino }] = await Promise.all([
  ...
  const destinoPorNome = new Map((exclusivosDestino ?? []).map((p) => [p.nome, p.id]));
  const produtoIdMap = new Map<string, string>();
  ...
  const [{ data: modulosOrigem }, { data: modulosDestino }, { data: planosOrigem }, { data: planosDestino }] = await Promise.all([
    supabase.from("modulos_produto").select("produto_id").eq("cenario_id", origemId),
    supabase.from("modulos_produto").select("produto_id").eq("cenario_id", cenarioId),
    supabase.from("planos_precificacao").select("*").eq("cenario_id", origemId),
    supabase.from("planos_precificacao").select("produto_id").eq("cenario_id", cenarioId),
  ]);
  const comModuloNoDestino = new Set((modulosDestino ?? []).map((m) => m.produto_id));
  const produtosSemModulo = new Set(
    (modulosOrigem ?? []).map((m) => m.produto_id as string).filter((pid) => !comModuloNoDestino.has(mapProdutoId(pid))),
  );
  if (produtosSemModulo.size > 0) await clonarModulos(supabase, origemId, cenarioId, mapProdutoId, falhas, produtosSemModulo);

  const comPlanoNoDestino = new Set((planosDestino ?? []).map((p) => p.produto_id));
  for (const plano of (planosOrigem ?? []) as Row[]) {
    if (comPlanoNoDestino.has(mapProdutoId(plano.produto_id))) continue;
    const { data: novoPlano } = await supabase
      .from("planos_precificacao")
      .insert({ ...semColunasDeSistema(plano), produto_id: mapProdutoId(plano.produto_id), cenario_id: cenarioId })
      .select("id")
      .single();
    if (!novoPlano) {
    ...
    if (precosFase && precosFase.length > 0) {
      await supabase.from("planos_precificacao_fases").insert((precosFase as Row[]).map((pf) => ({ ...semColunasDeSistema(pf), plano_id: novoPlano.id })));
    }
  }
  ...
  const [{ data: fasesOrigem }, { data: fasesDestino }] = await Promise.all([
    supabase.from("fases_produto").select("id, produto_id, fase").eq("cenario_id", origemId),
    supabase.from("fases_produto").select("id, produto_id, fase").eq("cenario_id", cenarioId),
  ]);
  const faseDestinoPorChave = new Map((fasesDestino ?? []).map((f) => [`${f.produto_id}__${f.fase}`, f.id as string]));
  for (const fo of fasesOrigem ?? []) {
    const faseDestinoId = faseDestinoPorChave.get(`${mapProdutoId(fo.produto_id)}__${fo.fase}`);
    if (!faseDestinoId) continue;
    const [{ data: funilOrigem }, { data: funilDestino }] = await Promise.all([
      supabase.from("premissas_funil").select("*").eq("fase_produto_id", fo.id).maybeSingle(),
      supabase.from("premissas_funil").select("*").eq("fase_produto_id", faseDestinoId).maybeSingle(),
    ]);
    if (!funilOrigem) continue;
    const dadosOrigem = semColunasDeSistema(funilOrigem as Row);
    delete dadosOrigem.fase_produto_id;
    if (!funilDestino) {
      await supabase.from("premissas_funil").insert({ ...dadosOrigem, fase_produto_id: faseDestinoId });
      continue;
    }
    const lacunas: Row = {};
    ...
    if (Object.keys(lacunas).length > 0) await supabase.from("premissas_funil").update(lacunas).eq("id", (funilDestino as Row).id);
  }

  await copiarConfiguracoesDeCenario(supabase, origemId, cenarioId, mapProdutoId);
