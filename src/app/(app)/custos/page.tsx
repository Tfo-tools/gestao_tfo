...
export default async function LancamentosPage() {
  ...
  const [{ data: planoContas }, { data: produtos }, { data: despesas }] = await Promise.all([
    supabase
      .from("plano_contas")
    ...
    supabase
      .from("despesas")
      .select("id, data_gasto, valor_total, comprovado, plano_contas:plano_contas_id(codigo, conta), produtos:produto_id(nome)")
  ...
  return (
    <div className="grid grid-cols-[420px_1fr] items-start gap-5">
      <DespesaForm planoContas={planoContas ?? []} produtos={produtos ?? []} />
