...
function montarParametrosEscalonado(formData: FormData): ResultadoParametros {
  ...
  if (baseado_em === "fase") {
    const produto_referencia_id = String(
      formData.get("produto_referencia_id") || "",
    );
    if (!produto_referencia_id)
      return {
        ok: false,
        erro: "Selecione o produto de referência da fase da empresa.",
      };
    const faixasPorFase = FASES.map((f) => {
      const raw = formData.get(`fase_${f.value}`);
      return raw !== null && raw !== ""
        ? { fase: f.value, valor: Number(raw) }
        : null;
    }).filter(
      (f): f is { fase: (typeof FASES)[number]["value"]; valor: number } =>
        f !== null,
    );
    if (faixasPorFase.length === 0)
      return { ok: false, erro: "Informe o valor de ao menos uma fase." };
  ...
  const maximos = formData
    .getAll("faixa_maximo")
    .map((v) => (v === "" ? null : Number(v)));
  const valores = formData.getAll("faixa_valor").map(Number);
  const faixas = minimos
    .map((minimo, i) => ({ minimo, maximo: maximos[i], valor: valores[i] }))
    .filter((f) => !isNaN(f.minimo) && !isNaN(f.valor));
  if (faixas.length === 0)
    return { ok: false, erro: "Adicione ao menos uma faixa." };
...
function montarRateio(formData: FormData): Record<string, unknown> {
  ...
  if (exclusivo)
    return { rateio_modo: "manual", rateio_manual: { [exclusivo]: 1 } };
  if (
    rateio_modo !== "auto_clientes" &&
    rateio_modo !== "auto_receita" &&
    rateio_modo !== "manual"
  )
    return {};
  if (rateio_modo === "auto_clientes" || rateio_modo === "auto_receita")
    return { rateio_modo };
...
function montarFolha(formData: FormData): Record<string, unknown> {
  return formData.get("folha") ? { folha: true } : {};
}
...
function montarParametros(
  ...
  if (tipo_custo === "escalonado") {
    ...
    return {
      ...
      parametros: {
        ...base.parametros,
        ...montarRateio(formData),
        ...montarFolha(formData),
      },
    };
  }
  ...
  if (tipo_custo === "cronograma") {
    ...
    if (!mes_inicio || !valoresRaw)
      return {
        ok: false,
        erro: "Preencha o mês de início e os valores mensais.",
      };
    const valores_mensais = valoresRaw
      .split(",")
      .map((v) => Number(v.trim()))
      .filter((v) => !isNaN(v));
    if (valores_mensais.length === 0)
      return {
        ok: false,
        erro: "Informe os valores mensais separados por vírgula.",
      };
...
export async function criarCustoEmpresa(
  ...
  const tipo_custo = String(
    formData.get("tipo_custo") || "",
  ) as TipoCustoEmpresa;
...
// Preço padrão pesquisado (contador, jurídico…) guardado fora dos cenários. Aplicar copia o modelo
// como um custos_empresa comum — dali em diante é editável no cenário sem mexer no modelo.

export async function aplicarModeloMercado(
  modeloId: string,
  cenarioId: string,
  ...
  const { data: modelo } = await supabase
    .from("modelos_custo_mercado")
    .select("*")
    .eq("id", modeloId)
    .single();
  if (!modelo || !cenarioId)
    return { error: "Modelo ou cenário não encontrado." };

  const { error } = await supabase.from("custos_empresa").insert({
    cenario_id: cenarioId,
    item: modelo.nome,
    plano_contas_id: modelo.plano_contas_id,
    tipo_custo: modelo.tipo_custo,
    valor_mensal: modelo.valor_mensal,
    data_inicio: dataInicio || null,
    data_fim: null,
    parametros: modelo.parametros,
    observacoes:
      `Aplicado do modelo de mercado. ${modelo.observacoes ?? ""}`.trim(),
  });
  if (error) return { error: "Não foi possível aplicar o modelo." };
...
/** Guarda um custo do cenário como modelo de mercado (substitui o modelo de mesmo nome). */
export async function salvarComoModeloMercado(
  custoId: string,
  fonte: string | null,
): Promise<ActionState> {
  const supabase = await createClient();
  ...
  const { error } = await supabase.from("modelos_custo_mercado").upsert(
    {
      nome: custo.item,
      plano_contas_id: custo.plano_contas_id,
      tipo_custo: custo.tipo_custo,
      valor_mensal: custo.valor_mensal,
      parametros: custo.parametros ?? {},
      fonte,
...
export async function excluirModeloMercado(modeloId: string): Promise<void> {
  const supabase = await createClient();
  await supabase.from("modelos_custo_mercado").delete().eq("id", modeloId);
  revalidatePath("/plano-de-custos/empresa");
}
