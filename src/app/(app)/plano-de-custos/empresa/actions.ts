...
export async function criarCustoEmpresa(
  ...
  if (tipo_custo === "fixo") {
  ...
  } else if (tipo_custo === "escalonado") {
    parametros.baseado_em = String(formData.get("baseado_em") || "receita");
    ...
    if (faixas.length === 0) return { error: "Adicione ao menos uma faixa." };
    parametros.faixas = faixas;
  } else if (tipo_custo === "cronograma") {
    const mes_inicio = String(formData.get("mes_inicio") || "");
    const valoresRaw = String(formData.get("valores_mensais") || "");
    if (!mes_inicio || !valoresRaw) return { error: "Preencha o mês de início e os valores mensais." };
    const valores_mensais = valoresRaw
      .split(",")
      .map((v) => Number(v.trim()))
      .filter((v) => !isNaN(v));
    if (valores_mensais.length === 0) return { error: "Informe os valores mensais separados por vírgula." };
