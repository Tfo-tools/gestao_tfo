...
export async function salvarReceitaHistorica(_prev: ActionState, formData: FormData): Promise<ActionState> {
  const id = String(formData.get("id") || "") || null;
  const cenario_id = String(formData.get("cenario_id") || "");
  const descricao = String(formData.get("descricao") || "").trim();
  const valor_mensal = Number(String(formData.get("valor_mensal") || "").replace(",", "."));
  ...
  if (!Number.isFinite(valor_mensal) || valor_mensal <= 0) return { error: "Informe o valor mensal." };
  ...
  if (data_fim && data_fim < data_inicio) return { error: "O fim não pode ser antes do início." };

  const supabase = await createClient();
  const dados = { cenario_id, descricao, valor_mensal, data_inicio, data_fim, observacoes };
  const { error } = id
