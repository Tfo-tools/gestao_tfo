...
import { LABEL_STATUS, STATUS_NO_PLANO, type StatusProduto } from "@/lib/fases-produto";
import { SeloStatus } from "@/app/(app)/produtos/status-produto";
import { InfoTooltip } from "@/components/info-tooltip";

type Item = { id: string; nome: string; status: StatusProduto; selecionado: boolean };
      ...
      {ehBase ? (
        <>
          <p className="mb-4 text-[11px] text-text-muted">
            Plano da empresa: entram automaticamente os produtos aprovados ou iniciados. A escolha se faz pelo status,
            em <Link href="/produtos" className="underline">Produtos</Link>.
          </p>
          <div className="flex flex-col gap-1.5">
            {noPlano.length === 0 && (
              <p className="text-[12px] text-text-faint">
                Nenhum produto aprovado ainda — enquanto isso, o plano da empresa fica vazio.
              </p>
            )}
            {noPlano.map((p) => (
              <div key={p.id} className="flex items-center justify-between rounded-md border border-border-soft px-2.5 py-2">
                <span className="text-[12px] font-medium">{p.nome}</span>
                <div className="flex items-center gap-2">
                  <SeloStatus status={p.status} />
                  <span className="text-[10px] text-text-faint">entrou por status</span>
                </div>
              </div>
            ))}
          </div>
          {produtos.length > noPlano.length && (
            <p className="mt-3 text-[10.5px] text-text-faint">
              ...
              {produtos
                ...
                .map((p) => `${p.nome} (${LABEL_STATUS[p.status].toLowerCase()})`)
                .join(", ")}
              .
            </p>
          )}
        ...
        <>
          <p className="mb-4 text-[11px] text-text-muted">
            Marque o que este cenário simula. O que ficar desmarcado é ignorado no recálculo, sem erro.
          </p>
          <div className="flex flex-col gap-1.5">
            {produtos.length === 0 && <p className="text-[12px] text-text-faint">Nenhum produto cadastrado ainda.</p>}
            {produtos.map((p) => (
              <label
                key={p.id}
                className={`flex cursor-pointer items-center justify-between rounded-md border px-2.5 py-2 ${
                  p.selecionado ? "border-primary-fill bg-primary-soft" : "border-border-soft"
                }`}
              >
                  ...
                  <input
                    ...
                    onChange={() =>
                      startTransition(async () => {
                        const r = await alternarProdutoNoCenario(cenarioId, p.id, !p.selecionado);
                        setErro(r.error);
                      })
                    }
                  />
