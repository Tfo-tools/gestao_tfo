...
import {
  duplicarAcaoMarketing,
  excluirAcaoMarketing,
  salvarAcaoMarketing,
} from "./acoes-marketing-actions";
...
function formatMes(iso: string) {
  return new Date(iso.slice(0, 7) + "-01T12:00:00").toLocaleDateString(
    "pt-BR",
...
function planoDeLinha(
  plano: string,
): Pick<RetornoAcao, "plano_tipo" | "plano_nome"> {
...
function acaoDoRascunho(r: Rascunho): AcaoMarketing {
  return {
    ...
    retorno: r.retorno.map((l) => ({
      produto_id: l.produto_id,
      clientes: Number(l.clientes || 0),
      ...planoDeLinha(l.plano),
    })),
    parametros: {
      cpl: numOuNull(r.cpl),
      conversao:
        numOuNull(r.conversao) != null
          ? Number(numOuNull(r.conversao)) / 100
          : null,
      canal: r.canal,
    },
...
export function FeirasEventos({
  ...
  function novo(tipo: TipoAcao) {
    ...
    setRascunho({
      ...
      retorno:
        tipo === "campanha"
          ? [{ produto_id: produtos[0]?.id ?? "", plano: "", clientes: "1" }]
          : [],
    });
  }
  ...
  function editar(a: AcaoMarketing) {
    ...
    const temPartes =
      p.estande != null || p.logistica != null || p.material != null;
    setRascunho({
      id: a.id,
      tipo: a.tipo,
      nome: a.nome,
      ...
      conversao:
        p.conversao != null
          ? String(Number((p.conversao * 100).toFixed(3)))
          : "",
      ...
      retorno: (a.retorno ?? []).map((r) => ({
        ...
        plano:
          r.plano_tipo && r.plano_nome ? `${r.plano_tipo}:${r.plano_nome}` : "",
    ...
    startTransition(async () => {
      const res = await salvarAcaoMarketing(cenarioId, {
        ...
        parametros:
          r.tipo === "feira"
            ? {
                estande: numOuNull(r.estande),
                logistica: numOuNull(r.logistica),
                material: numOuNull(r.material),
              }
            : r.tipo === "campanha"
              ? {
                  canal: r.canal,
  ...
  function excluir(a: AcaoMarketing) {
    if (
      !confirm(
        `Excluir "${a.nome}"? O custo e as vendas previstas saem da projeção.`,
      )
    )
      return;
    startTransition(async () => {
  ...
  return (
    <div className="mt-4 rounded-lg border border-primary-fill/50 bg-primary-soft/20 p-4">
      <div className="mb-2 flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 max-w-3xl text-[11px] text-text-muted">
            <b>Feira:</b> estande + logística + material, provisionado em{" "}
            {PARCELAS_FEIRA} parcelas mensais no ano dela (vendas no mês da
            feira). <b>Eventos:</b> custo do ano vira um fixo mensal.{" "}
            <b>Campanha:</b> verba mensal ÷ custo por lead × conversão =
            clientes por mês.
          </p>
        </div>
        {!rascunho && (
          <div className="flex shrink-0 flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={() => novo("evento")}
              className="rounded-lg border border-border bg-surface px-3 py-1.5 text-[12px] font-medium text-primary-deep"
            >
              + Eventos do ano
            </button>
            <button
              type="button"
              onClick={() => novo("campanha")}
              className="rounded-lg border border-border bg-surface px-3 py-1.5 text-[12px] font-medium text-primary-deep"
            >
              + Campanha de mídia
            </button>
      ...
      {acoes.length === 0 && !rascunho && (
        <p className="text-[11.5px] text-text-faint">
          Nenhuma ação de marketing cadastrada neste cenário.
        </p>
      )}
            ...
            <div className="mb-1.5 flex items-baseline justify-between border-b border-border-soft pb-1">
              ...
              {ano !== "sem data" && (
                <span className="text-[11px] text-text-muted">
                  custo das ações no ano: {formatBRL(custoNoAno(ano))}
                </span>
              )}
            </div>
            <div className="flex flex-col gap-2">
              {lista.map((a) => {
                ...
                return (
                  <div
                    key={a.id}
                    className="rounded-lg border border-border bg-surface px-3 py-2.5 text-[12px]"
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <span className="mr-1.5 rounded bg-bg px-1.5 py-0.5 text-[10px] font-semibold uppercase text-text-muted">
                          {LABEL_TIPO_ACAO[a.tipo]}
                        </span>
                        ...
                        <p className="mt-1 text-[11px] text-text-muted">
                          ...
                          {a.tipo === "feira"
                            ? ` → ${formatBRL(total / PARCELAS_FEIRA)}/mês de ${formatMes(meses[0])} a ${formatMes(meses[meses.length - 1])} (provisão) · vendas em ${a.mes ? formatMes(a.mes) : "—"}`
                            : a.tipo === "evento"
                          ...
                          {clientes > 0 &&
                            ` · ${formatNum(clientes)} cliente${clientes === 1 ? "" : "s"} · custo por cliente ${formatBRL(total / clientes)}`}
                        </p>
                        {a.tipo === "feira" &&
                          (p.estande != null ||
                            p.logistica != null ||
                            ...
                            <p className="mt-0.5 text-[11px] text-text-faint">
                              Estande {formatBRL(Number(p.estande ?? 0))} ·
                              logística {formatBRL(Number(p.logistica ?? 0))} ·
                              material {formatBRL(Number(p.material ?? 0))}
                            </p>
                          )}
                        {(a.retorno ?? []).length > 0 &&
                          a.tipo !== "campanha" && (
                            <p className="mt-0.5 text-[11px] text-text-faint">
                              Retorno{" "}
                              {a.tipo === "feira" ? "na feira" : "por evento"}:{" "}
                              {a.retorno
                                .map(
                                  (r) =>
                                    `${formatNum(Number(r.clientes))} ${nomeProduto.get(r.produto_id) ?? "produto"}${r.plano_nome ? ` (${r.plano_nome})` : ""}`,
                                )
                                .join(" · ")}
                            </p>
                          )}
                          ...
                          (a.retorno ?? []).length > 0 && (
                            <p className="mt-0.5 text-[11px] text-text-faint">
                              ...
                              {a.retorno
                                .map(
                                  (r) =>
                                    `${nomeProduto.get(r.produto_id) ?? "produto"}${r.plano_nome ? ` (${r.plano_nome})` : ""}`,
                                )
                                .join(", ")}{" "}
                              · CPL {formatBRL(Number(p.cpl ?? 0))} · conversão{" "}
                              {formatNum(Number(p.conversao ?? 0) * 100)}%
                            </p>
                          )}
                        {acaoSemRetorno(a) && (
                          <p className="mt-1 text-[11px] text-warning">
                            Sem retorno cadastrado — entra só o custo, e a
                            cobertura do canal direto não conta esta ação. Edite
                            e informe os clientes esperados.
                          </p>
                        )}
                      </div>
                      <div className="flex shrink-0 gap-2.5">
                        ...
                        <button
                          ...
                          onClick={() => duplicar(a)}
                          className="text-[11px] text-primary-deep"
                          title="Copia para o ano seguinte — ajuste o que mudar"
                        >
                          Duplicar p/{" "}
                          {Number(anoDaAcao(a) || new Date().getFullYear()) + 1}
                        </button>
                        <button
                          type="button"
                          disabled={isPending}
        ...
        <div className="mt-3 flex flex-col gap-3 rounded-lg border border-border bg-surface p-4">
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-[2fr_1fr_1fr_1fr_1fr]">
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Febratex Summit 2027"
                />
              </div>
              <div>
                <label className="mb-1 block text-[10.5px] font-medium text-text-muted">
              ...
              <div>
                ...
                <input
                  ...
                  min={0}
                  step="100"
                  value={rascunho.estande}
                  onChange={(e) => set("estande", e.target.value)}
                />
              </div>
              <div>
                <label className="mb-1 block text-[10.5px] font-medium text-text-muted">
                ...
                <input
                  ...
                  min={0}
                  step="100"
                  value={rascunho.logistica}
                  onChange={(e) => set("logistica", e.target.value)}
                  placeholder="viagem, hospedagem"
                />
              </div>
              <div>
                <label className="mb-1 block text-[10.5px] font-medium text-text-muted">
                ...
                <input
                  ...
                  min={0}
                  step="100"
                  value={rascunho.material}
                  onChange={(e) => set("material", e.target.value)}
                  placeholder="totens, brindes"
                />
              </div>
            </div>
          )}
          {rascunho.tipo === "feira" && (
            <p className="-mt-1 text-[10.5px] text-text-faint">
              ...
              {rascunho.mes
                ? ` → ${formatBRL(previa.custo / PARCELAS_FEIRA)}/mês de jan a dez de ${rascunho.mes.slice(0, 4)} (provisão) · vendas em ${formatMes(`${rascunho.mes}-01`)}`
                : ` provisionado em ${PARCELAS_FEIRA} parcelas mensais no ano da feira`}
            </p>
          )}
          ...
          {rascunho.tipo === "evento" && (
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-[2fr_1fr_1fr_1fr]">
                <div>
                  ...
                  <input
                    ...
                    value={rascunho.nome}
                    onChange={(e) => set("nome", e.target.value)}
                    placeholder="Ex: Eventos com associações"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[10.5px] font-medium text-text-muted">
              ...
              {rascunho.custo && rascunho.quantidade && (
                <p className="-mt-1 text-[10.5px] text-text-faint">
                  {formatBRL(
                    Number(rascunho.custo) * Number(rascunho.quantidade),
                  )}{" "}
                  no ano →{" "}
                  {formatBRL(
                    (Number(rascunho.custo) * Number(rascunho.quantidade)) / 12,
                  )}
                  /mês de custo fixo de eventos
                </p>
              )}
            </>
          )}
            ...
            <>
              <div className="grid grid-cols-2 gap-3 md:grid-cols-[2fr_1.3fr_1fr_1fr]">
                <div>
                  ...
                  <input
                    ...
                    value={rascunho.nome}
                    onChange={(e) => set("nome", e.target.value)}
                    placeholder="Ex: Google Search — Mind"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-[10.5px] font-medium text-text-muted">
                ...
                <div>
                  ...
                  <input
                    className="input"
                    type="month"
                    value={rascunho.mes_fim}
                    onChange={(e) => set("mes_fim", e.target.value)}
                  />
                </div>
              </div>
              ...
              {clientesMensaisCampanha(previa) > 0 && (
                <p className="-mt-1 rounded-md bg-bg px-2.5 py-1.5 text-[11px] text-text-muted">
                  ≈ <b>{formatNum(leadsMensaisCampanha(previa))} leads</b> e{" "}
                  <b>
                    {formatNum(clientesMensaisCampanha(previa))} clientes por
                    mês
                  </b>{" "}
                  · custo por cliente da campanha{" "}
                  <b>
                    {formatBRL(
                      Number(previa.custo) / clientesMensaisCampanha(previa),
                    )}
                  </b>{" "}
                  · {mesesDaCampanha(previa, fimCenario).length} meses · total{" "}
                  {formatBRL(custoTotalAcao(previa, fimCenario))} e{" "}
                  {formatNum(clientesTotaisAcao(previa, fimCenario))} clientes
                </p>
              )}
            </>
          )}
            ...
            <div className="flex flex-col gap-2">
              {rascunho.retorno.map((l, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`grid items-center gap-2 ${rascunho.tipo === "campanha" ? "grid-cols-[1.2fr_1.6fr_auto]" : "grid-cols-[1.2fr_1.6fr_0.7fr_auto]"}`}
                  >
                    <select
                      className="input"
                      value={l.produto_id}
                      onChange={(e) =>
                        atualizar({ produto_id: e.target.value, plano: "" })
                      }
                    >
                    ...
                    {rascunho.tipo !== "campanha" && (
                      <input
                        ...
                        value={l.clientes}
                        onChange={(e) =>
                          atualizar({ clientes: e.target.value })
                        }
                      />
                    )}
                    <button
                      ...
                      onClick={() =>
                        set(
                          "retorno",
                          rascunho.retorno.filter((_, j) => j !== i),
                        )
                      }
                  ...
                  onClick={() =>
                    set("retorno", [
                      ...rascunho.retorno,
                      {
                        ...
                        plano: "",
                        clientes: rascunho.tipo === "campanha" ? "1" : "",
                      },
                    ])
                  }
          ...
          <label className="flex items-start gap-2 rounded-md bg-bg px-2.5 py-2 text-[11.5px]">
            <input
              type="checkbox"
              className="mt-0.5"
              checked={rascunho.soma_na_meta}
              onChange={(e) => set("soma_na_meta", e.target.checked)}
            />
            <span>
              <b>Somar esses clientes à projeção.</b> Deixe desmarcado no
              normal: os clientes da ação fazem parte da meta do canal direto e
              aparecem no quadro de cobertura. Marque só se a ação traz clientes
              além do crescimento definido nas fases — senão eles contam duas
              vezes.
            </span>
          </label>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Ex: estande compartilhado com a associação"
            />
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="rounded-lg border border-border px-3.5 py-2 text-[12.5px] text-text-muted"
            >
              Cancelar
            </button>
          </div>
      ...
      {erro && !rascunho && (
        <p className="mt-2 text-[11.5px] text-danger">{erro}</p>
      )}
    </div>
  );
}
