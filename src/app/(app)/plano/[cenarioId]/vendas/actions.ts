...
export async function salvarPontoPartida(
  cenarioId: string,
  ...
  if (!ponto)
    return {
      error:
        "Este cenário não tem ponto de partida — ele acumula clientes desde o início de cada produto.",
    };

  for (const [produtoId, valor] of Object.entries(clientesPorProduto)) {
    if (!ponto.produtos[produtoId] || !Number.isFinite(valor) || valor < 0)
      continue;
  ...
  return falhas.length > 0
    ? { error: `Salvo, mas a projeção falhou em: ${falhas.join("; ")}` }
    : { error: null, success: true };
}

/** Busca de novo, no cenário de origem, os clientes do mês anterior ao início (descarta edições). */
export async function recarregarPontoPartida(
  cenarioId: string,
): Promise<CurvaActionState> {
  const supabase = await createClient();
  const { data: cenario } = await supabase
    .from("cenarios")
    .select("data_inicio, cenario_origem_id")
    .eq("id", cenarioId)
    .single();
  if (!cenario?.cenario_origem_id || !cenario.data_inicio)
    return { error: "Este cenário não foi espelhado de outro." };

  const ponto = await capturarPontoPartida(
    supabase,
    cenario.cenario_origem_id,
    cenario.data_inicio,
  );
  if (!ponto)
    return {
      error:
        "O cenário de origem não tem projeção no mês anterior ao início deste — recalcule a origem primeiro.",
    };
  const { error } = await supabase
    .from("cenarios")
    .update({ ponto_partida: ponto })
  ...
  revalidatePath("/relatorios");
  return falhas.length > 0
    ? { error: `Atualizado, mas a projeção falhou em: ${falhas.join("; ")}` }
    : { error: null, success: true };
}
...
export async function salvarFasesDatas(
  ...
  const travado = (
    (statusRows ?? []) as { id: string; nome: string; status: StatusProduto }[]
  ).find(
    (p) => datasTravadas(p.status) && linhas.some((l) => l.produto_id === p.id),
  );
  if (travado) {
    return {
...
export async function salvarPlanejamentoFase(
  ...
  const supabase = await createClient();
  const falhas: string[] = [];
  await gravarPlanejamentoDaFase(supabase, cenario_id, fase, linhas, falhas);

  if (falhas.length > 0) {
    return { error: `Salvo, exceto: ${falhas.join("; ")}.` };
  }
  revalidarPlanejamento(cenario_id);
  return { error: null, success: true };
}

/** Grade inteira de Crescimento & Churn: todas as fases de todos os produtos num salvar só. */
export async function salvarPlanejamentoGrade(
  _prevState: CurvaActionState,
  formData: FormData,
): Promise<CurvaActionState> {
  const cenario_id = String(formData.get("cenario_id") || "");
  let porFase: { fase: string; linhas: LinhaPlanejamento[] }[];
  try {
    porFase = JSON.parse(String(formData.get("fases") || "[]"));
  } catch {
    return { error: "Não foi possível ler a grade." };
  }
  if (!cenario_id) return { error: "Dados incompletos." };

  const supabase = await createClient();
  const falhas: string[] = [];
  for (const f of porFase)
    await gravarPlanejamentoDaFase(
      supabase,
      cenario_id,
      f.fase,
      f.linhas,
  ...
  if (falhas.length > 0) {
    return { error: `Salvo, exceto: ${[...new Set(falhas)].join("; ")}.` };
  }
  revalidarPlanejamento(cenario_id);
  return { error: null, success: true };
}

function revalidarPlanejamento(cenario_id: string) {
  revalidatePath(`/plano/${cenario_id}/vendas`);
  revalidatePath("/contratacoes/necessidade");
  revalidatePath("/relatorios");
}

async function gravarPlanejamentoDaFase(
  ...
  for (const linha of linhas) {
    ...
    const nomeProduto =
      ((faseRow?.produtos as any)?.nome as string) ?? "produto";

    if (!faseRow) {
      // Fase sem data em Produtos: não há onde gravar. Só é erro se veio algo preenchido.
      const temAlgo =
        linha.taxa_crescimento_mensal != null ||
        linha.taxa_churn_mensal != null ||
        (linha.trimestres ?? []).some(
          (t) =>
            t.taxa_crescimento_mensal != null || t.taxa_churn_mensal != null,
        );
      if (temAlgo)
    ...
    if (linha.capacidade_vendedor_mes != null)
      premissas.capacidade_vendedor_mes = linha.capacidade_vendedor_mes;
    if (linha.reunioes_por_oportunidade != null)
      premissas.reunioes_por_oportunidade = linha.reunioes_por_oportunidade;
    if (linha.span_of_control != null)
      premissas.span_of_control = linha.span_of_control;
    if (linha.horas_suporte_por_cliente_mes != null)
      premissas.horas_suporte_por_cliente_mes =
        linha.horas_suporte_por_cliente_mes;
    if (Object.keys(premissas).length > 0) {
    ...
    if (linha.trimestres) {
      const preenchidos = linha.trimestres.filter(
        (t) => t.taxa_crescimento_mensal != null || t.taxa_churn_mensal != null,
      );
      const vazios = linha.trimestres
        .filter(
          (t) =>
            t.taxa_crescimento_mensal == null && t.taxa_churn_mensal == null,
        )
        ...
        const { error: triError } = await supabase
          ...
          .upsert(
            preenchidos.map((t) => ({
              ...
              indice: t.indice,
              taxa_crescimento_mensal: t.taxa_crescimento_mensal,
              taxa_churn_mensal: t.taxa_churn_mensal,
            })),
