...
export default async function PlanoVendasPage({ params }: { params: Promise<{ cenarioId: string }> }) {
  const { cenarioId } = await params;
  const supabase = await createClient();

  const [{ data: cenario }, { data: produtos }] = await Promise.all([
    supabase.from("cenarios").select("id, nome, data_inicio, data_fim, ponto_partida").eq("id", cenarioId).single(),
  ...
  const capacidadePorModelo = new Map((modelosContratacao ?? []).map((m) => [m.id, (m.parametros as any)?.capacidade_unidade_mes ?? 0]));
  ...
  const [{ data: parceirosRaw }, { data: canalProdutosRaw }] =
    canalIds.length > 0
      ? await Promise.all([
          supabase.from("canal_parceiros_fase").select("canal_id, fase, quantidade_parceiros").in("canal_id", canalIds),
  ...
  const produtosData: ProdutoCurva[] = await Promise.all(
    (produtos ?? []).map(async (produto) => {
      const { data: fasesRaw } = await supabase
        .from("fases_produto")
        .select("id, fase, taxa_crescimento_mensal, taxa_churn_mensal, data_inicio, data_fim")
      ...
      const { data: funis } =
        faseIds.length > 0
          ? await supabase.from("premissas_funil").select("fase_produto_id, capacidade_vendedor_mes, reunioes_por_oportunidade, span_of_control, horas_suporte_por_cliente_mes").in("fase_produto_id", faseIds)
          : { data: [] };
      const funilByFaseId = new Map((funis ?? []).map((f) => [f.fase_produto_id, f]));
      const { data: trimestresRaw } =
        faseIds.length > 0
          ? await supabase.from("fases_trimestres").select("fase_produto_id, indice, taxa_crescimento_mensal, taxa_churn_mensal").in("fase_produto_id", faseIds)
          : { data: [] };
          ...
          return {
            ...
            dados: fase
              ? {
                  ...
                  capacidade_vendedor_mes: funil?.capacidade_vendedor_mes ?? null,
                  reunioes_por_oportunidade: funil?.reunioes_por_oportunidade ?? null,
                  span_of_control: funil?.span_of_control ?? null,
                  horas_suporte_por_cliente_mes: funil?.horas_suporte_por_cliente_mes ?? null,
  ...
  function faseDoMes(produtoId: string, mesIso: string): string | null {
    ...
    return candidatas[0] ? (faseLabel.get(candidatas[0].fase) ?? candidatas[0].fase) : null;
  }
  ...
  const linhasProjecao: LinhaProjecao[] = (projecaoRaw ?? []).filter((l) => dentroDoPeriodo(l.mes_referencia)).map((l) => ({
  ...
  return (
    <div>
      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <h1 className="font-heading text-[22px] font-semibold">Vendas — {cenario.nome}</h1>
          <p className="mt-1 text-[13px] text-text-muted">
            Defina canais, crescimento e churn nos painéis abaixo — a projeção no rodapé responde na hora.
          </p>
        </div>
      ...
      <div className="flex flex-col gap-3">
        ...
        <details className="group rounded-xl border border-border bg-surface">
          <summary className="flex cursor-pointer items-center gap-2 px-5 py-3.5 text-[13px] font-semibold">
            ...
            <span className="ml-2 text-[11px] font-normal text-text-muted">
              {canais.length > 0 ? canais.map((c) => c.nome).join(" · ") : "nenhum canal ainda"}
            </span>
          </summary>
        ...
        <TabelaProjecao linhas={linhasProjecao} produtos={produtos ?? []} cenarioId={cenarioId} />
      </div>
    </div>
  );
}
