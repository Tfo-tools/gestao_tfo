...
export async function criarPrograma(
  ...
  const valor_total = valorTotalRaw && valorTotalRaw !== "" ? Number(valorTotalRaw) : null;
  const valorPropostoRaw = formData.get("valor_proposto");
  const valor_proposto = valorPropostoRaw && valorPropostoRaw !== "" ? Number(valorPropostoRaw) : null;
  ...
  if (!nome || !tipo || (!valor_total && !valor_proposto)) {
    return { error: "Preencha nome, tipo e o valor total (ou o valor proposto, se ainda não foi aprovado)." };
  }
...
export async function confirmarValorAprovado(
  ...
  const supabase = await createClient();
  const { error } = await supabase.from("programas_investimento").update({ valor_total }).eq("id", id);
...
function deslocarData(dataIso: string | null | undefined, deltaDias: number): string | null {
...
export async function criarCenarioParaPrograma(programaId: string): Promise<{ error: string | null; cenarioId?: string }> {
  const supabase = await createClient();
  ...
  if (!dataAlvo) return { error: "Preencha a data prevista de assinatura do programa antes de gerar o plano." };

  const { data: base } = await supabase.from("cenarios").select("id").eq("is_base", true).maybeSingle();
  if (!base) return { error: "Nenhum cenário marcado como Plano Base — marque um cenário como base em Cenários primeiro." };
  ...
  if (!primeiraDataBase) return { error: "O Plano Base ainda não tem fases de produto cadastradas." };

  const deltaDias = Math.round((new Date(dataAlvo + "T00:00:00").getTime() - new Date(primeiraDataBase + "T00:00:00").getTime()) / 86400000);
  ...
  const { data: novoCenario, error } = await supabase
    ...
    .select("id")
    .single();
  if (error || !novoCenario) return { error: "Não foi possível criar o cenário." };

  const { data: fasesOrigem } = await supabase
    .from("fases_produto")
    .select("id, produto_id, fase, data_inicio, data_fim, taxa_crescimento_mensal, taxa_churn_mensal, observacoes")
    .eq("cenario_id", base.id);
  ...
  for (const fase of fasesOrigem ?? []) {
    ...
    const [{ data: funilOrigem }, { data: fixosOrigem }, { data: variaveisOrigem }, { data: alocacoesOrigem }] = await Promise.all([
      supabase.from("premissas_funil").select("*").eq("fase_produto_id", fase.id),
      supabase.from("plano_custos_fixos").select("plano_contas_id, tipo, item, quantidade, valor_unitario").eq("fase_produto_id", fase.id),
      supabase
      ...
      supabase.from("equipe_alocada").select("cargo, categoria, quantidade_funcionarios, horas_mes, custo_hora").eq("fase_produto_id", fase.id),
    ]);
    ...
    if (fixosOrigem && fixosOrigem.length > 0) {
      await supabase.from("plano_custos_fixos").insert(fixosOrigem.map((f) => ({ ...f, fase_produto_id: novaFase.id })));
    }
    if (variaveisOrigem && variaveisOrigem.length > 0) {
      await supabase.from("plano_custos_variaveis").insert(variaveisOrigem.map((v) => ({ ...v, fase_produto_id: novaFase.id })));
    }
    if (alocacoesOrigem && alocacoesOrigem.length > 0) {
      await supabase.from("equipe_alocada").insert(alocacoesOrigem.map((a) => ({ ...a, fase_produto_id: novaFase.id })));
    }
  }
  ...
  await supabase.from("cenario_programas").insert({ cenario_id: novoCenario.id, programa_id: programaId });
