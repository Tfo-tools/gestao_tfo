  ...
  return (
    <div>
      <div className="mb-7 flex items-center justify-between">
        <div>
          <h1 className="font-heading text-[22px] font-semibold">Funil &amp; Metas</h1>
          <p className="mt-1 text-[13px] text-text-muted">
            Dimensionamento de equipe comercial e CAC all-in por produto e fase
          </p>
        </div>
        {cenarioNome && (
      ...
      <div className="grid grid-cols-3 gap-4">
        {(produtos ?? []).map((p) => (
          <Link
            key={p.id}
            href={cenarioAtual ? `/funil/${p.id}?cenario=${cenarioAtual}` : `/funil/${p.id}`}
